export const AdminClass = {
  container: 'bg-[#F9F9F9] min-h-[95vh] py-10',
  wrapper: 'container mx-auto flex flex-col gap-[20px]',
  title: 'text-2xl font-medium capitalize',
  searchInputContainer: 'flex items-center justify-between flex-wrap',
  inputContainer: 'flex items-center space-x-[16px]',
  searchInput:
    'border border-[#F0F0F0] rounded-sm py-[11px] px-[16px] w-[240px] text-[14px] font-light text-[#474C52]',
  dropdown:
    'rounded-sm border border-[#F0F0F0] py-[11px] min-w-[150px] px-5 text-[14px] text-[#474C52] bg-white font-normal',
  exportBtn:
    'border border-[#4D4D4D] py-[11px] px-[25px] rounded-[100px] text-[16px] font-medium text-[#1e1e1e] flex items-center gap-2',
  actionBtnsContainer: 'flex items-center space-x-[16px]',
  addBtn:
    'bg-[#1e1e1e] rounded-[100px] py-[10px] px-[25px] font-medium text-white',
};

export const TableClass = {
  table: 'w-full table-auto border-collapse mb-14',
  thead: 'bg-[#F7F7F7] text-left',
  tbody: 'bg-[#FFFFFF]',
  tr: '',
  th: 'py-3 sm:py-[21px] px-3 sm:px-[28px] text-left text-xs sm:text-[14px] text-[#1E1E1E] font-semibold whitespace-nowrap',
  td: 'py-3 sm:py-[21px] px-3 sm:px-[28px] text-left text-xs sm:text-[13px] text-[#1E1E1E] font-light border-b border-[#F0F0F0]',
  dropdown:
    'rounded-sm border border-[#F0F0F0] py-3 sm:py-[10px] min-w-[110px] sm:min-w-[130px] px-3 sm:px-5 text-xs sm:text-[14px] text-[#474C52] bg-white font-normal',
};
